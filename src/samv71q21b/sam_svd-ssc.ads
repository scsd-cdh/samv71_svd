...
package SAM_SVD.SSC is
   ...
   type RCMR_CKSSelect is
     (--  Divided Clock
      ...
      RK)
     with Size => 2;
   for RCMR_CKSSelect use
     (MCK => 0,
      TK => 1,
      RK => 2);

   --  Receive Clock Output Mode Selection
   type RCMR_CKOSelect is
     (--  None, RK pin is an input
      ...
      TRANSFER)
     with Size => 3;
   for RCMR_CKOSelect use
     (NONE => 0,
      CONTINUOUS => 1,
      TRANSFER => 2);

   --  Receive Clock Gating Selection
   type RCMR_CKGSelect is
     (--  None
      CONTINUOUS,
      ...
      EN_RF_HIGH)
     with Size => 2;
   for RCMR_CKGSelect use
     (CONTINUOUS => 0,
      EN_RF_LOW => 1,
      EN_RF_HIGH => 2);

   --  Receive Start Selection
   type RCMR_STARTSelect is
...
--  end of transfer of the previous data.
   ...
   type SSC_RCMR_Register is record
      --  Receive Clock Selection
      CKS            : RCMR_CKSSelect := SAM_SVD.SSC.MCK;
      --  Receive Clock Output Mode Selection
      CKO            : RCMR_CKOSelect := SAM_SVD.SSC.NONE;
      --  Receive Clock Inversion
      CKI            : Boolean := False;
      --  Receive Clock Gating Selection
      CKG            : RCMR_CKGSelect := SAM_SVD.SSC.CONTINUOUS;
      --  Receive Start Selection
      START          : RCMR_STARTSelect := SAM_SVD.SSC.CONTINUOUS;
   ...
   type RFMR_FSEDGESelect is
     (--  Positive Edge Detection
      POSITIVE,
      --  Negative Edge Detection
      NEGATIVE)
     with Size => 1;
   for RFMR_FSEDGESelect use
     (POSITIVE => 0,
      NEGATIVE => 1);
   ...
   type SSC_RFMR_Register is record
      ...
      --  Receive Frame Sync Output Selection
      FSOS           : RFMR_FSOSSelect := SAM_SVD.SSC.NONE;
      --  unspecified
      Reserved_23_23 : HAL.Bit := 16#0#;
      --  Frame Sync Edge Detection
      FSEDGE         : RFMR_FSEDGESelect := SAM_SVD.SSC.POSITIVE;
      --  unspecified
   ...
   type TCMR_CKSSelect is
     (--  Divided Clock
      ...
      TK)
     with Size => 2;
   for TCMR_CKSSelect use
     (MCK => 0,
      RK => 1,
      TK => 2);

   --  Transmit Clock Output Mode Selection
   type TCMR_CKOSelect is
     (--  None, TK pin is an input
      ...
      TRANSFER)
     with Size => 3;
   for TCMR_CKOSelect use
     (NONE => 0,
      CONTINUOUS => 1,
      TRANSFER => 2);

   --  Transmit Clock Gating Selection
   type TCMR_CKGSelect is
     (--  None
      CONTINUOUS,
      ...
      EN_TF_HIGH)
     with Size => 2;
   for TCMR_CKGSelect use
     (CONTINUOUS => 0,
      EN_TF_LOW => 1,
      EN_TF_HIGH => 2);

   --  Transmit Start Selection
   type TCMR_STARTSelect is
...
--  enabled), and immediately after the end of transfer of the previous data
   ...
   type SSC_TCMR_Register is record
      --  Transmit Clock Selection
      CKS            : TCMR_CKSSelect := SAM_SVD.SSC.MCK;
      --  Transmit Clock Output Mode Selection
      CKO            : TCMR_CKOSelect := SAM_SVD.SSC.NONE;
      --  Transmit Clock Inversion
      CKI            : Boolean := False;
      --  Transmit Clock Gating Selection
      CKG            : TCMR_CKGSelect := SAM_SVD.SSC.CONTINUOUS;
      --  Transmit Start Selection
      START          : TCMR_STARTSelect := SAM_SVD.SSC.CONTINUOUS;
      --  unspecified
      Reserved_12_15 : HAL.UInt4 := 16#0#;
   ...
   --  Frame Sync Edge Detection
   type TFMR_FSEDGESelect is
     (--  Positive Edge Detection
      POSITIVE,
      --  Negative Edge Detection
      NEGATIVE)
     with Size => 1;
   for TFMR_FSEDGESelect use
     (POSITIVE => 0,
      NEGATIVE => 1);
   ...
   type SSC_TFMR_Register is record
      ...
      --  Transmit Frame Sync Output Selection
      FSOS           : TFMR_FSOSSelect := SAM_SVD.SSC.NONE;
      --  Frame Sync Data Enable
      FSDEN          : Boolean := False;
      --  Frame Sync Edge Detection
      FSEDGE         : TFMR_FSEDGESelect := SAM_SVD.SSC.POSITIVE;
      --  unspecified
      Reserved_25_27 : HAL.UInt3 := 16#0#;
      --  FSLEN Field Extension
